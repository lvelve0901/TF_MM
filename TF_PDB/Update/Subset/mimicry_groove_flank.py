...
mminfos = []
for i in range(-2,2):
    mminfos.append(pd.read_csv("../../../Paper/Data/cutoff_mm_value_groove_sigma%.1f_flank_%s.csv"%(sigma,i)))
        ...
        for target, seq in zip(targets,seqs):
            ...
            for pairid, value, flank, mminfo in zip(pairids,values,flanks,mminfos):
                ...
                for mm in mmlist:
                    ...
                    #if pdbid == '5zk1' and mm == 'CC':
                    #    print((pairid_o,param,flank,value))
...
print(output_df)
output_df.to_csv("mimicry_groove_flank_sigma%.1f.csv"%sigma,index=False,float_format='%.3f')
