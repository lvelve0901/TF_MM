...
output = [['tf','resi','flank','pdbid','pairid','isHbond','isVdw','param','value','std_refine']]
...
for idx, param in enumerate(bp_params):
    ...
    for i in range(len(pdbinfo)):
        ...
        #for resi in range(1,len(subdf)-1):
            ...
            for pairid, value, flank in zip(pairids,values,flanks):
                ...
                output.append([name,resi,flank,pdbid,pairid_o,isHbond,isVdw,param,value,std_refine])

output_df = pd.DataFrame(output[1:],columns=output[0])
print(output_df)
output_df.to_csv("features_all_abg_flank.csv",index=False,float_format='%.3f')
