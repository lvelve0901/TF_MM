...
output = [['tf','sequence_star','resi','pdbid','pairid','isHbond','isVdw']]
...
for i in range(len(pdbinfo)):
    ...
    for target, seq in zip(targets,seqs):
        ...
        output.append([name,sequence_star,resi,pdbid,pairid,isHbond,isVdw])


output_df = pd.DataFrame(output[1:],columns=output[0])
print(output_df)
output_df.to_csv("position_bp.csv",index=False,float_format='%.3f')
