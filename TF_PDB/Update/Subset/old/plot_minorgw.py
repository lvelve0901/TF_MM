...
pdblist = ['1an2','1dux','1hjb','1nkp','1nlw','1p47','1qne','1r4r','2stw','3gat','3kz8','5t00','5u01','5zk1','6qhd']
lblist =   [
            [[8.5,9.5],[11.5,12.5]],
            [[5.5,6.5]],
            [[13.5,14.5],[16.5,17.5]],
...
for idx, param in enumerate(bp_params):
    ...
    for i in range(len(pdbinfo)):
        ...
        param_value = np.array(param_value) - 5.8

        ylim_min = np.floor(np.min([ylower3,np.nanmin(param_value)]))
        ylim_max = np.ceil(np.max([yupper3,np.nanmax(param_value)]))
        ...
        ax1.plot(seqrange+0.5,param_value,'-o',color='blue',
