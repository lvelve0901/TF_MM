...
output = [['tf','sequence_star','resi','pdbid','pairid','isHbond','isVdw','param','value']]
    ...
    for i in range(len(pdbinfo)):
        ...
        for target, seq in zip(targets,seqs):
            ...
            if value <= ylower1 or value >= yupper1:
                output.append([name,sequence_star,resi,pdbid,pairid,isHbond,isVdw,param,value])

output_df = pd.DataFrame(output[1:],columns=output[0])
print(output_df)
output_df.to_csv("distortion_abg_sigma%.1f.csv"%sigma,index=False,float_format='%.3f')
