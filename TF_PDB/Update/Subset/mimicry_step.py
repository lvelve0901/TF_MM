...
mminfo = pd.read_csv("../../../Paper/Data/final_cutoff_mm_value_bp_step_sigma%.1f.csv"%sigma)
...
output = [['tf','sequence_star','strand1','strand2','resi','pdbid','pairid','isHbond','isVdw','param','value','mismatch']]
...
for idx, param in enumerate(bp_params):
    ...
    for i in range(len(pdbinfo)):
        ...
        for target, seq in zip(targets,seqs):
            ...
            pairid = subdf.ix[resi-1]['pair_id']
            mmlist = target[1]
            value = param_value[resi]
            ...
            for mm in mmlist:
                isSign = mminfo.loc[mminfo.mismatch == mm][param].values
                ...
                if isSign == -1.0:
                    if value <= ylower1:
                        output.append([name,sequence_star,strand1,strand2,resi,pdbid,pairid,isHbond,isVdw,param,value,mm])
                elif isSign == 1.0:
                    if value >= yupper1:
                        output.append([name,sequence_star,strand1,strand2,resi,pdbid,pairid,isHbond,isVdw,param,value,mm])
                elif isSign == 2.0:
...
print(output_df)
output_df.to_csv("mimicry_step_sigma%.1f.csv"%sigma,index=False,float_format='%.3f')
