...
params = ['beta','gamma','zeta']


for i, param in enumerate(params):

    print("--- Working on parameters: %s [%d/%d] ---"%(param,i,len(params)))
    output = [['tf_name','tf','seq_type','core','strand1','strand2','position','sim_type','mismatch','mm_id','binding','pos1_avg','pos1_std','pos2_avg','pos2_std','pos3_avg','pos3_std']]
    ...
    for j, tf in enumerate(tf_list):
        ...
        nt1_list = range(position-1,position+2)

        df_temp = pd.DataFrame([],columns=['resi','value'])
        df_temp['resi'] = pd.Series(nt1_list)
        ...
        for i in range(len(mismatches)):
            ...
            df = pd.read_table("/home/hs189/MMproj/TF_MD/ABG/%s/%s/abglist.txt"%(tf,folder),delim_whitespace=True)
            if tf_name == 'tbp':
                subdf = df.loc[(df.resi.isin(nt1_list))]
            else:
                subdf = df.loc[(df.resi.isin(nt1_list)) & (df.RMSD1 < rmsd_max) & (df.RMSD2 < rmsd_max)]

            df_mean = subdf.groupby(['resi'])[param].mean().reset_index().rename(columns={param:"mean"}) 
            df_std = subdf.groupby(['resi'])[param].std().reset_index().rename(columns={param:"std"})
            df_param = pd.concat([df_mean,df_std['std']],axis=1)
            t = df_temp.merge(df_param,left_on='resi',right_on='resi',how='left')
            mean = t['mean'].values
            std = t['std'].values
            pos1_avg, pos2_avg, pos3_avg = mean
            pos1_std, pos2_std, pos3_std = std
            
            output.append([tf_name,tf,seq_type,core,strand1,strand2,position,sim_type,mismatch,mm_id,binding,pos1_avg,pos1_std,pos2_avg,pos2_std,pos3_avg,pos3_std])
